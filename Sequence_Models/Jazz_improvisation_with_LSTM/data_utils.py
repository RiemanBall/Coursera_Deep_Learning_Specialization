...
def predict_and_sample(inference_model, 
    ...
    """
    results = np.zeros((Ty, 78))
    indices = np.zeros((Ty, 1))
    
    inference_model.reset_states()
    
    for i in range(Ty):
        predictions = inference_model(x_initializer)
        ...
        x_initializer = tf.expand_dims([predicted_id], 0)

        indices[i, 0] = predicted_id
